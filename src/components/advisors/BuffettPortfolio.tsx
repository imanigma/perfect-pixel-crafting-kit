...
import { getWarrenBuffettPortfolio, calculateInvestment, StockPortfolio } from "@/utils/portfolioScraper";
...
export const BuffettPortfolio = () => {
  ...
  const fetchPortfolio = async () => {
    setIsLoading(true);
    try {
      const portfolioData = await getWarrenBuffettPortfolio();
  ...
  return (
    <Card className="bg-[#151515] border-[#333945] text-white w-full">
      <CardHeader>
        <CardTitle className="text-xl">Warren Buffett's Portfolio Analyzer</CardTitle>
        <CardDescription className="text-[#8E9196]">
          See how your investment would be allocated following Buffett's strategy
        </CardDescription>
      </CardHeader>
      <CardContent>
      ...
      <CardFooter className="border-t border-[#333945] text-[#8E9196] text-sm">
        Data scraped from CNBC's Berkshire Hathaway portfolio page. Allocation is based on reported holdings percentages.
      </CardFooter>
    </Card>
  );
};
